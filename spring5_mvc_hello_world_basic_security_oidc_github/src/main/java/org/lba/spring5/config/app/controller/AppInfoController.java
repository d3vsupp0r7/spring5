...
public class AppInfoController {
	...
	public String infoContextBeans(Model model) {
		...
		// http://localhost:8080/spring5_mvc_hello_world_basic_security_oidc_github/infoContextBeans
	...
	public String infoContextBeansByName(@RequestParam String beanName, Model model) {
		//	http://localhost:8080/spring5_mvc_hello_world_basic_security_oidc_github/infoContextBeansByName?beanName=employeeServiceImpl
	...
	@RequestMapping("/infoContextBeansByLikeName")
	public String infoContextBeansByLikeName(@RequestParam String beanName, Model model) {
		//	http://localhost:8080/spring5_mvc_hello_world_basic_security_oidc_github/infoContextBeansByLikeName?beanName=security
		...
		for (String currentBeanName : beanNames) {
			...
			if(currentBeanName.contains(beanName)) {
				logger.debug(currentBeanName + " : " + applicationContext.getBean(currentBeanName).getClass().toString());
				AppInfoBean bean = new AppInfoBean(i,currentBeanName,applicationContext.getBean(currentBeanName).getClass().toString()); 
				/**/
				beanInContextList.add(bean);
